...
final class RemoteSocket(
  ...
  lishogikka.shutdown(shutdown, _.PhaseBeforeServiceUnbind, "Telling lishogi-ws we're stopping") { () =>
    request[Unit](
      id => send(Protocol.Out.stop(id)),
      res => logger.info(s"lishogi-ws says: $res")
    ).withTimeout(1 second)
      .addFailureEffect(e => logger.error("lishogi-ws stop", e))
      .nevermind
  }
